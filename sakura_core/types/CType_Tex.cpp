...
{
	//名前と拡張子
	pType->m_szTypeName = L"TeX";
	pType->m_szTypeExts = L"tex,ltx,sty,bib,log,blg,aux,bbl,toc,lof,lot,idx,ind,glo";
...
public:
	...
	void each(TagProcessor<HierarchyCount>&& process)
	{
		...
		for (CLogicInt nLineLen, nLineNumber = CLogicInt(0); nLineNumber < nLineCount; ++nLineNumber) {
			const wchar_t* const pLine = CDocLine::GetDocLineStrWithEOL(refDocLineMgr.GetLine(nLineNumber), &nLineLen);
