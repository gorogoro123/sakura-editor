...
{
	pType->m_szTypeName = L"HTML";
	pType->m_szTypeExts = L"html,htm,shtml,plg";
...
void CDocOutline::MakeTopicList_html(CFuncInfoArr* pcFuncInfoArr, bool bXml)
{
	...
	*/
	...
	{
		pLineBuf = CDocLine::GetDocLineStrWithEOL(m_pcDocRef->m_cDocLineMgr.GetLine(nLineCount), &nLineLen);
		if( !pLineBuf )
		{
			break;
		}
