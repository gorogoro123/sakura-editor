...
void CType_Cobol::InitTypeConfigImp(STypeConfig* pType)
{
	...
	pType->m_szIndentChars = L"*";						/* その他のインデント対象文字 */
...
//			MYTRACE( L"szLabel=[%ls]\n", szLabel );
			...
			*/
			...
			auto_snprintf_s( szWork, std::size(szWork), L"%ls::%ls", szDivision, szLabel );
...
static const auto g_ppszKeywordsCOBOL = std::to_array<const wchar_t*>({
	L"ACCEPT",
	L"ADD",
	L"ADVANCING",
	L"AFTER",
