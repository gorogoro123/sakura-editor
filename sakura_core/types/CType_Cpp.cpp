...
/* del end 2005/12/6 じゅうじ	*/
				...
				if( C_IsWordChar( pLine[i] ) ){
							...
							if( pLine[i + 1] == L':' || (0 < i && pLine[i-1] == L':') ){
								...
								if(nMode2 == M2_NAMESPACE_SAVE)
								{
									...
									if (szWord[0] != '\0' && !is_final_context(szWord, szItemName))
										wcscpy_s( szItemName, std::size(szItemName), szWord );
					...
					if( nMode2 == M2_NAMESPACE_SAVE ){
						if(is_final_context(szWord, szItemName)){
							...
							wcscpy_s( szTemplateName, std::size(szTemplateName), szWord );
							wcscpy_s( szItemName, std::size(szItemName), szWord );
						}
						...
						if( wcscmp(L"final", szWord) != 0 || nNestLevel_template != 0 ){
							...
							if( 0 < nLen && C_IsWordChar(szTemplateName[nLen - 1]) && szTemplateName[nLen - 1] != L':' && szWord[nWordIdx] != L':' ){
								...
								if( nLen + 1 < nItemNameLenMax ){
									wcscat_s( szTemplateName, std::size(szTemplateName), L" " );
									nLen++;
								}
							}
					...
					else if( nNestLevel_func == 0 && (nMode2 == M2_NORMAL || nMode2 == M2_FUNC_NAME_END) )	// 2010.07.08 ryoji 関数型マクロ呼出しを関数と誤認することがある問題対策として nMode2 == M2_FUNC_NAME_END 条件を追加し、補正がかかるようにした。
					{
						...
						if( nItemFuncId != 0 && nItemFuncId != FL_OBJ_FUNCTION )	//  2010.07.08 ryoji nMode2 == M2_FUNC_NAME_END のときは nItemFuncId == 2 のはず
						{
							...
							wcscpy_s(szItemName, std::size(szItemName), LS(STR_OUTLINE_CPP_NONAME));
						}
					}
				...
				if( C_IsWordChar( pLine[i] ) ||
					...
					if( nMode2 == M2_TEMPLATE || nMode2 == M2_TEMPLATE_SAVE || nMode2 == M2_TEMPLATE_WORD ){
						if( pLine[i] == L'<' ){
							...
							wcscpy_s( szItemName, std::size(szItemName), szTemplateName );
							nNestLevel_template--;
							if( nNestLevel_template == 0 ){
								if( nMode2 == M2_TEMPLATE ){
									...
									nMode2 = nMode2Old;
									if( nMode2 == M2_OPERATOR_WORD ){
										wcscpy_s( szWord, std::size(szWord), szTemplateName);
			...
			if( M1_INIT == nMode ){
				...
				if( '{' == pLine[i] )
				{
					...
					// +1は終端NUL文字
					{
						const auto offset = nNamespaceLen[nNestLevel_global];
						wcscpy_s( &szNamespace[offset], std::size(szNamespace) - offset, szItemName);
						szItemName[0] = L'\0';
						...
						{
							++ nNestLevel_global;
							const auto offset1 = nNamespaceLen[nNestLevel_global] = nNamespaceLen[nNestLevel_global-1] + nItemNameLen;
							if( nItemFuncId == FL_OBJ_NAMESPACE ) {
								wcscpy_s(&szNamespace[offset1], std::size(szNamespace) - offset1, LS(STR_OUTLINE_CPP_DEFPOS));
							}
							else
							{
								szNamespace[offset1] = L'\0';
								szNamespace[offset1+1] = L':';
						...
						if(nNestLevel_fparam==0)
						{
							...
							if( bAdd ){
								if( szTemplateName[0] ){
									...
									wcscpy_s(szItemName, std::size(szItemName), szWordPrev);
								}
								nItemLine = nLineCount + CLogicInt(1);
							}
						}
							...
							if( //	前の文字列の末尾チェック
								...
								if( nMode2 == M2_NORMAL || nMode2 == M2_OPERATOR_WORD ){
									if( szTemplateName[0] ){
										...
										wcscpy_s( szWord, std::size(szWord), szWordPrev );
									}
								}
...
void CEditView::SmartIndent_CPP( wchar_t wcChar )
{
	...
	case L'(':
	{
		...
		pLine = CDocLine::GetDocLineStrWithEOL(m_pcEditDoc->m_cDocLineMgr.GetLine(GetCaret().GetCaretLogicPos().GetY2()), &nLineLen);
		if( nullptr == pLine ){
			if( WCODE::CR != wcChar ){
				return;
			}
		...
		for( j = GetCaret().GetCaretLogicPos().GetY2(); j >= CLogicInt(0); --j ){
			pLine2 = CDocLine::GetDocLineStrWithEOL(m_pcEditDoc->m_cDocLineMgr.GetLine(j), &nLineLen2);
