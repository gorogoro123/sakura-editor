			...
			/* 単語読み込み中 */
				...
				if( ! WCODE::IsBlank(pLine[i]) &&
					...
					if( 0 == wcscmp( L"class", szWordPrev ) ||
						...
						++nClassNestArrNum;
						if( 0 < nNestLevel	){
							wcscat_s( szClass, std::size(szClass), L"\\" );
						}
						wcscat_s( szClass, std::size(szClass), szWord );
			...
			if( FL_JAVA_MODE_NORMAL == nMode ){
				...
				loop_is_func:;
					...
					if( k >= nLineLen2 ){
						k = 0;
						++nLineCount2;
						pLine2 = CDocLine::GetDocLineStrWithEOL(m_pcDocRef->m_cDocLineMgr.GetLine(nLineCount2), &nLineLen2);
						if( nullptr != pLine2 ){
							goto loop_is_func;
						}
						if( 0 < nClassNestArrNum ){
