...
void CDlgDiff::SetData( )
{
	...
	/* 編集中のファイル一覧を作成する */
	{
		...
		if( nRowNum > 0 )
		{
			...
			SplitPath_FolderAndFile(m_szFile1, {}, szFile1);
			for( i = 0; i < nRowNum; i++ )
			{
				...
				SplitPath_FolderAndFile(pFileInfo->m_szPath, {}, szFile2);
...
int CDlgDiff::GetData( )
{
	...
	{
		ApiWrap::DlgItem_GetText( GetHwnd(), IDC_EDIT_DIFF_DST, m_szFile2, m_szFile2.capacity() );
