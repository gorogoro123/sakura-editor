...
void CDlgPluginOption::SetData( )
{
	...
	// タイトル
	auto_snprintf_s( buf, std::size(buf), LS(STR_DLGPLUGINOPT_TITLE), m_cPlugin->m_sName.c_str());
	...
	for( i=0, it = m_cPlugin->m_options.cbegin(); it != m_cPlugin->m_options.cend(); i++, it++ ){
		...
		if (cOpt->GetType() == OPTION_TYPE_BOOL) {
			...
			// 数値へ正規化
			auto_snprintf_s( buf, std::size(buf), L"%d", _wtoi(sValue.c_str()));
		}
	...
	for( i=0, it = m_cPlugin->m_options.cbegin(); it != m_cPlugin->m_options.cend(); i++, it++ ){
		...
		if (cOpt->GetType() == OPTION_TYPE_BOOL) {
			...
			for (auto it2 = selects.cbegin(); it2 != selects.cend(); it2++) {
				SepSelect(*it2, &sView, &sTrg);
				if (sView == sWbuf) {
					auto_snprintf_s( buf, std::size(buf), L"%ls", sTrg.c_str());
					break;
				}
			}
		}
...
void CDlgPluginOption::SetFromEdit( int iLine )
{
	...
	if (iLine >= 0) {
		...
		if (sType == OPTION_TYPE_BOOL) {
			...
			nVal = ::GetDlgItemInt( GetHwnd(), IDC_EDIT_PLUGIN_OPTION_NUM, nullptr, TRUE );
			auto_snprintf_s( buf, std::size(buf), L"%d", nVal);
		}
		else if (sType == OPTION_TYPE_SEL) {
