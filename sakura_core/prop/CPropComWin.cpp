...
//	BOOL	bRet;
	...
	ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_WINCAPTION_ACTIVE   ), m_Common.m_sWindow.m_szWindowCaptionActive.capacity() - 1 );	//@@@ 2003.06.13 MIK
	ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_WINCAPTION_INACTIVE ), m_Common.m_sWindow.m_szWindowCaptionInactive.capacity() - 1 );	//@@@ 2003.06.13 MIK
	...
	for (const auto& langInfo : CSelectLang::GetLangInfo()) {
		...
		if (langInfo->m_Path == m_Common.m_sWindow.m_szLanguageDll.c_str()) {
			nSelPos = uiIndex;
		}
		++uiIndex;
	}
...
int CPropWin::GetData( HWND hwndDlg )
{
	...
	ApiWrap::DlgItem_GetText( hwndDlg, IDC_WINCAPTION_ACTIVE, m_Common.m_sWindow.m_szWindowCaptionActive,
		m_Common.m_sWindow.m_szWindowCaptionActive.capacity() );
	ApiWrap::DlgItem_GetText( hwndDlg, IDC_WINCAPTION_INACTIVE, m_Common.m_sWindow.m_szWindowCaptionInactive,
		m_Common.m_sWindow.m_szWindowCaptionInactive.capacity() );

	// 言語選択
	HWND hwndCombo = ::GetDlgItem( hwndDlg, IDC_COMBO_LANGUAGE );
	int nSelPos = ApiWrap::Combo_GetCurSel( hwndCombo );
	if (const auto& langInfo = CSelectLang::GetLangInfo(nSelPos); langInfo.m_Path != m_Common.m_sWindow.m_szLanguageDll.c_str()) {
