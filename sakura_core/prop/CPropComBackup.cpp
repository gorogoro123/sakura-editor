...
//	int			nCharChars;
	...
	case WM_INITDIALOG:
		...
		ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_EDIT_BACKUPFOLDER ), m_Common.m_sBackup.m_szBackUpFolder.size() - 1 - 1 );
		// 20051107 aroka
		ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_EDIT_BACKUPFILE ), m_Common.m_sBackup.m_szBackUpPathAdvanced.size() - 1 - 1 );
		return TRUE;
	...
	case WM_COMMAND:
		...
		case EN_CHANGE: // 20051107 aroka フォルダーが変更されたらリアルタイムにエディットボックス内を更新
			...
			case IDC_EDIT_BACKUPFOLDER:
				...
				ApiWrap::DlgItem_GetText( hwndDlg, IDC_EDIT_BACKUPFOLDER, m_Common.m_sBackup.m_szBackUpFolder, m_Common.m_sBackup.m_szBackUpFolder.size() - 1 );
				UpdateBackupFile( hwndDlg );
				break;
			default:
				break;
