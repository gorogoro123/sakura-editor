...
//	int			nCharChars;
	...
	case WM_INITDIALOG:
		...
		ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_EDIT_BACKUPFOLDER ), m_Common.m_sBackup.m_szBackUpFolder.capacity() - 1 - 1 );
		// 20051107 aroka
		ApiWrap::EditCtl_LimitText( ::GetDlgItem( hwndDlg, IDC_EDIT_BACKUPFILE ), m_Common.m_sBackup.m_szBackUpPathAdvanced.capacity() - 1 - 1 );
		return TRUE;
	...
	case WM_COMMAND:
			...
			case IDC_BUTTON_BACKUP_FOLDER_REF:	/* フォルダー参照 */
				{
					...
					if( SelectDir( hwndDlg, LS(STR_PROPCOMBK_SEL_FOLDER), szFolder, szFolder ) ){
						m_Common.m_sBackup.m_szBackUpFolder = szFolder;
		...
		case EN_CHANGE: // 20051107 aroka フォルダーが変更されたらリアルタイムにエディットボックス内を更新
			...
			case IDC_EDIT_BACKUPFOLDER:
				...
				ApiWrap::DlgItem_GetText( hwndDlg, IDC_EDIT_BACKUPFOLDER, m_Common.m_sBackup.m_szBackUpFolder, m_Common.m_sBackup.m_szBackUpFolder.capacity() - 1 );
				UpdateBackupFile( hwndDlg );
				break;
			default:
				break;
...
//	}
	...
	ApiWrap::DlgItem_GetText( hwndDlg, IDC_EDIT_BACKUPFILE, m_Common.m_sBackup.m_szBackUpPathAdvanced, m_Common.m_sBackup.m_szBackUpPathAdvanced.capacity() - 1);
...
void CPropBackup::UpdateBackupFile(HWND hwndDlg)	//	バックアップファイルの詳細設定
{
	...
	if( !m_Common.m_sBackup.m_bBackUpPathAdvanced ){	// 詳細設定モードでないときだけ自動更新する
		auto_snprintf_s( m_Common.m_sBackup.m_szBackUpPathAdvanced, m_Common.m_sBackup.m_szBackUpPathAdvanced.capacity(), L"%ls", temp.c_str() );
		ApiWrap::DlgItem_SetText( hwndDlg, IDC_EDIT_BACKUPFILE, m_Common.m_sBackup.m_szBackUpPathAdvanced );
	}
	return;
}
