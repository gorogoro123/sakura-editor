...
// novice 2004/10/10
	...
	if( nFuncID != 0 ){
		...
		::PostMessage( ::GetParent( m_hwndParent ), WM_COMMAND, MAKELONG( nFuncID, CMD_FROM_MOUSE ),  (LPARAM)nullptr );
	}
//	/* 右クリックメニュー */
...
void CEditView::OnLBUTTONDBLCLK( WPARAM fwKeys, int _xPos , int _yPos )
{
	...
	if(! m_dwTripleClickCheck){
		...
		if( (CEditApp::getInstance()->GetGrepAgent()->GrepMode() || CAppMode::getInstance()->IsDebugMode()) && GetDllShareData().m_Common.m_sSearch.m_bGTJW_LDBLCLK ){
			/* タグジャンプ機能 */
