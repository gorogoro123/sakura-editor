...
) const
{
	*pnSearchStart = nPos;	// 2002.02.08 hor
	...
	if( m_sCurSearchOption.bRegularExp ){
		...
		** 2003.05.04 かろと
		*/
		if( m_CurRegexp.Match( cStr.data(), (int)cStr.size(), nPos ) ){
		...
		if( ! CWordParse::WhereCurrentWord_2( cStr.data(), CLogicInt(cStr.size()), nPos, GetDllShareData().m_Common.m_sEdit.m_bEnableExtEol, &posWordHead, &posWordEnd, nullptr, nullptr ) ) {
		...
		const wchar_t* pHit = CSearchAgent::SearchString(cStr.data(), (int)cStr.size(), nPos, m_sSearchPattern);
		if( pHit ){
			*pnSearchStart = pHit - cStr.data();
