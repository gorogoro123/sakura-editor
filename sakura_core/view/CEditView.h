...
public:
	...
	*/
	...
	int IsSearchString(std::span<const WCHAR> cStr, CLogicInt nPos, CLogicInt* pnSearchStart, CLogicInt* pnSearchEnd) const;	/* 現在位置が検索文字列に該当するか */	//2002.02.08 hor 引数追加
...
public:
	...
	BOOL MakeDiffTmpFile( SFilePath& filename, HWND hWnd, ECodeType code, bool bBom);	/* DIFF一時ファイル作成 */	//@@@ 2002.05.28 MIK	//2005.10.29 maru
	BOOL MakeDiffTmpFile2( SFilePath& tmpName, const WCHAR* orgName, ECodeType code, ECodeType saveCode);
...
public:
	...
	std::unique_ptr<CSplitBoxWnd>	m_pcsbwVSplitBox;	/* 垂直分割ボックス */
	std::unique_ptr<CSplitBoxWnd>	m_pcsbwHSplitBox;	/* 水平分割ボックス */
