...
)
{
	...
	*/
	...
	CLogicInt		nLineLen;
	const wchar_t*	pLine = CDocLine::GetDocLineStrWithEOL(m_pcEditDoc->m_cDocLineMgr.GetLine(ptXY.GetY2()), &nLineLen); //2007.10.09 kobake レイアウト・ロジック混在バグ修正
	...
	while( i <= ptXY.GetX2() && i < nLineLen ){
		bMatch = ( bUseRegexKeyword
					&& m_cRegexKeyword->RegexIsKeyword( std::span<const WCHAR>(pLine, nLineLen), i, &nUrlLen, &nMatchColor )
...
)
{
	...
	if( GetSelectionInfo().IsBoxSelecting() ){	/* 矩形範囲選択中 */
		...
		for( nLineNum = GetSelectionInfo().m_sSelect.GetFrom().GetY2(); nLineNum <= GetSelectionInfo().m_sSelect.GetTo().y; ++nLineNum ){
			...
			if( bWithLineNumber ){	/* 行番号を付与する */
				auto lineNumLen = auto_snprintf_s( strLineNum.data(), strLineNum.size() , L" %d:" , nLineNum + 1 );
				buffer->Append( pszSpaces, nLineNumCols - lineNumLen );
				buffer->Append( strLineNum.c_str(), (size_t)lineNumLen );
			}
