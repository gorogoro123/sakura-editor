...
class CJackManager final : public TSingleton<CJackManager>{
	friend TSingleton<CJackManager>;
	CJackManager();
...
public:
	...
	int GetCommandName( int funccode, std::span<WCHAR> buf ) const;	//プラグインコマンドの名前を返す
	int	GetCommandCount() const noexcept;	//プラグインコマンドの数を返す
