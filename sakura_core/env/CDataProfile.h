...
namespace profile_data {
	...
	/*!
	 * 指定した型の変換に対応しているかどうかを示すコンセプト
	 */
	template<class T>
	concept is_supported = std::is_integral_v<T> || std::is_enum_v<T> || std::is_same_v<T, CLogicInt> || std::is_same_v<T, CLayoutInt>;
	...
	{
		auto nValue = static_cast<int>(value);
