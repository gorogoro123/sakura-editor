...
//	MYTRACE( L"Iniファイル処理-1 所要時間(ミリ秒) = %d\n", cRunningTimer.Read() );
	...
	if( bRead ){
		...
		DWORD mH = 0; // ※ 古～い ini だと "szVersion" は無い
		DWORD mL = 0;
		DWORD lH = 0;
		DWORD lL = 0;
		if( cProfile.IOProfileData(L"Other", L"szVersion", iniVer) ) {
			swscanf_s( iniVer, L"%u.%u.%u.%u", &mH, &mL, &lH, &lL );
		}
...
void CShareData_IO::ShareData_IO_Mru( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].nViewTopLine", i );
		cProfile.IOProfileData( pszSecName, szKeyName, pfiWork->m_nViewTopLine );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].nViewLeftCol", i );
		cProfile.IOProfileData( pszSecName, szKeyName, pfiWork->m_nViewLeftCol );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].nX", i );
		cProfile.IOProfileData( pszSecName, szKeyName, pfiWork->m_ptCursor.x );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].nY", i );
		cProfile.IOProfileData( pszSecName, szKeyName, pfiWork->m_ptCursor.y );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].nCharCode", i );
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MRU[%02d].bFavorite", i );
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"ExceptMRU[%02d]", i );
...
void CShareData_IO::ShareData_IO_Keys( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"SEARCHKEY[%02d]", i );
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"REPLACEKEY[%02d]", i );
...
void CShareData_IO::ShareData_IO_Grep( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"GREPFILE[%02d]", i );
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"GREPFOLDER[%02d]", i );
	...
	for (i = 0; i < nSize; ++i) {
		auto_snprintf_s(szKeyName, std::size(szKeyName), L"GREPEXCLUDEFILE[%02d]", i);
	...
	for (i = 0; i < nSize; ++i) {
		auto_snprintf_s(szKeyName, std::size(szKeyName), L"GREPEXCLUDEFOLDER[%02d]", i);
...
void CShareData_IO::ShareData_IO_Cmd( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"szCmdArr[%02d]", i );
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"szCurDirArr[%02d]", i );
...
void CShareData_IO::ShareData_IO_Nickname( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"From%02d", i );
		cProfile.IOProfileData(pszSecName, szKeyName, pShare->m_Common.m_sFileName.m_szTransformFileNameFrom[i]);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"To%02d", i );
...
void CShareData_IO::ShareData_IO_Toolbar( CDataProfile& cProfile, CMenuDrawer* pcMenuDrawer )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"nTBB[%03d]", i );
...
void CShareData_IO::IO_CustMenu( CDataProfile& cProfile, CommonSetting_CustomMenu& menu, bool bOutCmdName)
{
	...
	for( i = 0; i < MAX_CUSTOM_MENU; ++i ){
		...
		for( j = 0; j < nSize; ++j ){
			...
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"nCMIF[%02d][%02d]", i, j );
			if (cProfile.IsReadingMode()) {
			...
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"nCMIK[%02d][%02d]", i, j );
	...
	for( i = 0; i < sKeyBind.m_nKeyNameArrNum; ++i ){
		...
		if( cProfile.IsReadingMode() ){
			if (bOldVer) {
				...
				KEYDATA tmpKeydata;
				auto_snprintf_s(szKeyName, std::size(szKeyName), L"KeyBind[%03d]", i);
				if( cProfile.IOProfileData(szSecName, szKeyName, StringBufferW(szKeyData)) ){
					wchar_t	*p;
					wchar_t	*pn;
					int		nRes;
...
// start 新バージョン	2008/5/25 Uchi
			KEYDATA& keydata = sKeyBind.m_pKeyNameArr[i];
			auto_snprintf_s(szKeyName, std::size(szKeyName), L"KeyBind[%03d]", i);
			auto_snprintf_s(szKeyData, std::size(szKeyData), L"%04x", keydata.m_nKeyCode);
			...
			for(int j = 0; j < 8; j++)
			{
				...
				if (GetPlugCmdInfoByFuncCode( keydata.m_nFuncCodeArr[j], szFuncName )) {
					// Plugin
					auto_snprintf_s( szWork, std::size(szWork), L",%ls", szFuncName );
				}
				else {
					if (bOutCmdName) {
						//@@@ 2002.2.2 YAZAKI マクロをCSMacroMgrに統一
			...
			if( 0x0100 <= keydata.m_nKeyCode ){
				auto_snprintf_s(szWork, std::size(szWork), L",%s", names[ keydata.m_nKeyCode - 0x0100 ]);
			}else{
...
void CShareData_IO::ShareData_IO_Print( CDataProfile& cProfile )
{
	...
	for( i = 0; i < MAX_PRINTSETTINGARR; ++i ){
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].nInts", i );
		...
		if( cProfile.IsReadingMode() ){
			...
			auto_snprintf_s( szKeyData, std::size(szKeyData), pszForm,
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szSName"	, i );
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_szPrintSettingName);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szFF"	, i );
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_szPrintFontFaceHan);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szFFZ"	, i );
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_szPrintFontFaceZen);
		// ヘッダー/フッター
		for( j = 0; j < 3; ++j ){
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szHF[%d]" , i, j );
			cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_szHeaderForm[j]);
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szFTF[%d]", i, j );
			cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_szFooterForm[j]);
		}
		{ // ヘッダー/フッター フォント設定
			WCHAR	szKeyName2[64];
			WCHAR	szKeyName3[64];
			auto_snprintf_s( szKeyName, std::size(szKeyName),  L"PS[%02d].lfHeader",			i );
			auto_snprintf_s( szKeyName2, std::size(szKeyName2), L"PS[%02d].nHeaderPointSize",	i );
			auto_snprintf_s( szKeyName3, std::size(szKeyName3), L"PS[%02d].lfHeaderFaceName",	i );
			...
			auto_snprintf_s( szKeyName, std::size(szKeyName),  L"PS[%02d].lfFooter",			i );
			auto_snprintf_s( szKeyName2, std::size(szKeyName2), L"PS[%02d].nFooterPointSize",	i );
			auto_snprintf_s( szKeyName3, std::size(szKeyName3), L"PS[%02d].lfFooterFaceName",	i );
			ShareData_IO_Sub_LogFont( cProfile, pszSecName, szKeyName,szKeyName2, szKeyName3,
				printsetting.m_lfFooter, printsetting.m_nFooterPointSize );
		}

		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szDriver", i );
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_mdmDevMode.m_szPrinterDriverName);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szDevice", i );
		cProfile.IOProfileData(pszSecName, szKeyName, printsetting.m_mdmDevMode.m_szPrinterDeviceName);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].szOutput", i );
		...
		//禁則	//@@@ 2002.04.09 MIK
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].bKinsokuHead", i ); cProfile.IOProfileData( pszSecName, szKeyName, printsetting.m_bPrintKinsokuHead );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].bKinsokuTail", i ); cProfile.IOProfileData( pszSecName, szKeyName, printsetting.m_bPrintKinsokuTail );
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].bKinsokuRet",  i ); cProfile.IOProfileData( pszSecName, szKeyName, printsetting.m_bPrintKinsokuRet );	//@@@ 2002.04.13 MIK
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].bKinsokuKuto", i ); cProfile.IOProfileData( pszSecName, szKeyName, printsetting.m_bPrintKinsokuKuto );	//@@@ 2002.04.17 MIK

		//カラー印刷
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"PS[%02d].bColorPrint", i ); cProfile.IOProfileData( pszSecName, szKeyName, printsetting.m_bColorPrint );	// 2013/4/26 Uchi
	}
}
...
void CShareData_IO::ShareData_IO_Types( CDataProfile& cProfile )
{
	...
	for( i = 0; i < pShare->m_nTypesCount; ++i ){
		auto_snprintf_s( szKey, std::size(szKey), L"Types(%d)", i );
...
void CShareData_IO::ShareData_IO_Type_One( CDataProfile& cProfile, STypeConfig& types, const WCHAR* pszSecName)
{
	...
	for(j = 0; j < MAX_VERTLINES; j++ ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"nVertLineIdx%d", j + 1 );
...
//@@@ 2001.11.17 add start MIK
	{	//正規表現キーワード
		...
		{
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"RxKey[%03d]", j );
			if( cProfile.IsReadingMode() )
			{
			...
			{
				auto_snprintf_s( szKeyData, std::size(szKeyData), L"%ls,%ls",
...
//		cProfile.IOProfileData( pszSecName, L"nKeyHelpNum", types.m_nKeyHelpNum );				/* 登録辞書数 */
		...
		for(j = 0; j < MAX_KEYHELP_FILE; j++){
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"KDct[%02d]", j );
			/* 読み出し */
			if( cProfile.IsReadingMode() ){
				...
				if(types.m_KeyHelpArr[j].m_szPath[0] != L'\0'){
					auto_snprintf_s( szKeyData, std::size(szKeyData), L"%d,%s,%s",
	...
	if( cProfile.IsReadingMode() ){
		...
		if( bIOSuccess ){
			...
			for( int i = 0; i < nKeyWordSetNum; ++i ){
				...
				int nKeyWordNum = 0;
				//値の取得
				auto_snprintf_s( szKeyName, std::size(szKeyName), L"szSN[%02d]", i );
				cProfile.IOProfileData(pszSecName, szKeyName, StringBufferW(szKeyData));
				auto_snprintf_s( szKeyName, std::size(szKeyName), L"nCASE[%02d]", i );
				cProfile.IOProfileData( pszSecName, szKeyName, bKEYWORDCASE );
				auto_snprintf_s( szKeyName, std::size(szKeyName), L"nKWN[%02d]", i );
				cProfile.IOProfileData( pszSecName, szKeyName, nKeyWordNum );

				//追加
				pCKeyWordSetMgr->AddKeyWordSet( szKeyData, bKEYWORDCASE, nKeyWordNum );
				auto_snprintf_s( szKeyName, std::size(szKeyName), L"szKW[%02d]", i );
		...
		for( int i = 0; i < nSize; ++i ){
			...
			auto_snprintf_s( szKeyName, std::size(szKeyName), L"szKW[%02d].Size", i );
...
void CShareData_IO::ShareData_IO_Macro( CDataProfile& cProfile )
{
	...
	for( i = 0; i < MAX_CUSTMACRO; ++i ){
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"Name[%03d]", i );
		cProfile.IOProfileData(pszSecName, szKeyName, StringBufferW(macrorec.m_szName));
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"File[%03d]", i );
		cProfile.IOProfileData(pszSecName, szKeyName, macrorec.m_szFile);
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"ReloadWhenExecute[%03d]", i );
...
void CShareData_IO::IO_MainMenu( CDataProfile& cProfile, std::vector<std::wstring>* pData, CommonSetting_MainMenu& mainmenu, bool bOutCmdName)
{
	...
	for (int i = 0; i < mainmenu.m_nMainMenuNum; i++) {
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"MM[%03d]", i );
		if (cProfile.IsReadingMode()) {
			// 読み込み時初期化
			...
			if (GetPlugCmdInfoByFuncCode( pcMenu->m_nFunc, szFuncName )) {
				...
				if ( !bOutCmdName || p == nullptr ) {
					auto_snprintf_s( szFuncName, std::size(szFuncName), L"%d", pcMenu->m_nFunc );
				}
			}
			// 書き込み
			// ラベル編集後のノードはノード名を出力する 2012.10.14 syat 各国語対応
			auto_snprintf_s( szLine, std::size(szLine), L"%d,%d,%ls,%ls,%ls", 
				pcMenu->m_nLevel, 
				pcMenu->m_nType, 
				szFuncName, 
				pcMenu->m_sKey, 
...
void CShareData_IO::ShareData_IO_Other( CDataProfile& cProfile )
{
	...
	for( i = 0; i < nSize; ++i ){
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"TagJumpKeyword[%02d]", i );
		if( i >= nSize ){
	...
	if( ! cProfile.IsReadingMode() ){
		WCHAR	iniVer[256];
		auto_snprintf_s( iniVer, std::size(iniVer), L"%d.%d.%d.%d", 
...
void CShareData_IO::IO_ColorSet( CDataProfile* pcProfile, const WCHAR* pszSecName, ColorInfo* pColorInfoArr )
{
	...
	for( j = 0; j < COLORIDX_LAST; ++j ){
		...
		auto_snprintf_s( szKeyName, std::size(szKeyName), L"C[%s]", g_ColorAttributeArr[j].szName );	//Stonee, 2001/01/12, 2001/01/15
...
{
	WCHAR szKey[64];
	auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).eItemType", i );
	...
	if( cProfile.IsReadingMode() || item.m_eFileTreeItemType == EFileTreeItemType_Grep
		...
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).szTargetPath", i );
	...
	if( cProfile.IsReadingMode()
		...
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).szLabelName", i );
		cProfile.IOProfileData( pszSecName, szKey, item.m_szLabelName );
	}
	auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).nDepth", i );
	cProfile.IOProfileData( pszSecName, szKey, item.m_nDepth );
	if( cProfile.IsReadingMode() || item.m_eFileTreeItemType == EFileTreeItemType_Grep ){
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).szTargetFile", i );
		cProfile.IOProfileData( pszSecName, szKey, item.m_szTargetFile );
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).bIgnoreHidden", i );
		cProfile.IOProfileData( pszSecName, szKey, item.m_bIgnoreHidden );
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).bIgnoreReadOny", i );
		cProfile.IOProfileData( pszSecName, szKey, item.m_bIgnoreReadOnly );
		auto_snprintf_s( szKey, std::size(szKey), L"FileTree(%d).bIgnoreSystem", i );
