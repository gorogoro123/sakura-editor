...
//	To Here Sept. 19,2000
		...
		// [タブバー]タブ
		{
			...
			sTabBar.m_szTabWndCaption = L"${w?【Grep】$h$:【アウトプット】$:$f$n$}${U?(更新)$}${R?(ビューモード)$:(上書き禁止)$}${M?【キーマクロの記録中】$}";
		...
		// [マクロ]タブ
		{
			...
			sMacro.m_szMACROFOLDER = szIniFolder;	/* マクロ用フォルダー */
			...
			*/
			{
				...
				*/
				...
				int i = 0;
				auto_snprintf_s( szSettingName, std::size(szSettingName), L"印刷設定 %d", i + 1 );
			...
			for( int i = 1; i < MAX_PRINTSETTINGARR; ++i ){
				...
				auto_snprintf_s( m_pShareData->m_PrintSettingArr[i].m_szPrintSettingName, m_pShareData->m_PrintSettingArr[i].m_szPrintSettingName.capacity(), L"印刷設定 %d", i + 1 );	/* 印刷設定の名前 */
			}
		}
