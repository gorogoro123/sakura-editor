...
{
	LockGuard<CMutex> guard( g_cDocTypeMutex );
	return FALSE != SendMessageW( m_pShareData->m_sHandles.m_hwndTray, MYWM_ADD_TYPESETTING, (WPARAM)cDocumentType.GetIndex(), 0 );
}

bool CDocTypeManager::DelTypeConfig(CTypeConfig cDocumentType)
{
	LockGuard<CMutex> guard( g_cDocTypeMutex );
	return FALSE != SendMessageW( m_pShareData->m_sHandles.m_hwndTray, MYWM_DEL_TYPESETTING, (WPARAM)cDocumentType.GetIndex(), 0 );
}
...
{
	StaticString<MAX_TYPES_EXTS> szWork(pszTypeExts);
	WCHAR* content = nullptr;
	WCHAR* token = wcstok_s(szWork, m_typeExtSeps, &content);
	while (token) {
		...
		token = wcstok_s(nullptr, m_typeExtSeps, &content);
	}
	return false;
}
