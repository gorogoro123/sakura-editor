...
public:
	//ファイル名関連
	void GetTransformFileNameFast( LPCWSTR, std::span<WCHAR>, HDC hDC, bool bFitMode = true, int cchMaxWidth = 0 );	// 2002.11.24 Moca Add
	int TransformFileName_MakeCache( void );
	static void GetFilePathFormat( std::wstring_view strSrc, LPWSTR pszDest, size_t nDestLen, std::wstring_view strFrom, std::wstring_view strTo );
	...
	bool GetMenuFullLabel_File(WCHAR* pszOutput, int nBuffSize, const WCHAR* pszFile, int id, HDC hDC, [[maybe_unused]] bool bModified = false, ECodeType nCharCode = CODE_NONE){
		return GetMenuFullLabel(pszOutput, nBuffSize, true, pszFile, id, false, nCharCode, false, -1, false, hDC);
	}
	bool GetMenuFullLabel_FileNoEscape(WCHAR* pszOutput, int nBuffSize, const WCHAR* pszFile, int id, HDC hDC, [[maybe_unused]] bool bModified = false, ECodeType nCharCode = CODE_NONE){
