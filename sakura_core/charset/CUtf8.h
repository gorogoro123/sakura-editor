...
public:
	...
	EConvertResult _UnicodeToHex(const wchar_t* cSrc, const int iSLen, std::span<WCHAR> szDst, const CommonSetting_Statusbar* psStatusbar, const bool CESU8Mode);			//!< UNICODE → Hex 変換
	EConvertResult UnicodeToHex(const wchar_t* ps, const int nsl, std::span<WCHAR> szDst, const CommonSetting_Statusbar* psStatusbar) override{ return _UnicodeToHex(ps, nsl, szDst, psStatusbar, false); }
