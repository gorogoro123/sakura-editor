...
void CControlTray::DoGrepCreateWindow(HINSTANCE hinst, HWND msgParent, CDlgGrep& cDlgGrep)
{
	...
	auto_snprintf_s( szTemp, std::size(szTemp), L"%d", cDlgGrep.m_nGrepCharSet );
			...
			case F_FILEOPEN:	/* 開く */
				{
					...
					for( const auto& file : files ){
						sLoadInfo.cFilePath = file.c_str();
...
int	CControlTray::CreatePopUpMenu_L( )
{
	...
	m_cMenuDrawer.MyAppendMenu( hMenuTop, MF_BYPOSITION | MF_STRING | MF_POPUP, (UINT_PTR)hMenu, L"TrayL", L'\0' );
	...
	if( j > 0 ){
		...
		for( i = 0; i < m_pShareData->m_sNodes.m_nEditArrNum; ++i ){
			if( IsSakuraMainWindow( m_pShareData->m_sNodes.m_pEditArr[i].GetHwnd() ) ){
				...
				m_cMenuDrawer.MyAppendMenu( hMenu, MF_BYPOSITION | MF_STRING, IDM_SELWINDOW + i, szMenu, L'\0', FALSE );
				++j;
			}
		}
	}
...
int	CControlTray::CreatePopUpMenu_R( )
{
	...
	hMenuTop = ::CreatePopupMenu();
	hMenu = ::CreatePopupMenu();
	m_cMenuDrawer.MyAppendMenu( hMenuTop, MF_BYPOSITION | MF_STRING | MF_POPUP, (UINT_PTR)hMenu, L"TrayR", L'\0' );
