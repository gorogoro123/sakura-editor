...
void CPrintPreview::OnPreviewGoDirectPage( )
{
	...
	auto_snprintf_s( szMessage, std::size(szMessage), LS(STR_ERR_DLGPRNPRVW4) , m_nAllPageNum );
	auto_snprintf_s( szPageNum, std::size(szPageNum), L"%d", m_nCurPageNum + 1 );
	...
	if( FALSE != bDlgInputPageResult ){
		int i;
		auto nPageNumLen = int(wcsnlen(szPageNum, std::size(szPageNum)));
...
void CPrintPreview::OnPreviewGoPage( int nPage )
{
	...
	wchar_t	szEdit[1024];
	auto_snprintf_s( szEdit, std::size(szEdit), LS(STR_ERR_DLGPRNPRVW6), m_nCurPageNum + 1, m_nAllPageNum );
	ApiWrap::DlgItem_SetText( m_hwndPrintPreviewBar, IDC_STATIC_PAGENUM, szEdit );

	auto_snprintf_s( szEdit, std::size(szEdit), L"%d %%", m_nPreview_Zoom );
...
//		MYTRACE( L"%s\n", szErrMsg );
	...
	for( i = 0; i < nNum; ++i ){
		...
		auto_snprintf_s( szProgress, std::size(szProgress), L"%d/%d", i + 1, nNum );
...
CColorStrategy* CPrintPreview::DrawPageTextFirst(int nPageNum)
{
	...
	if (m_pPrintSetting->m_bColorPrint) {
		...
		if (pcPageTopLayout != nullptr) {
			...
			if (nPageTopOff) {
				std::span<const WCHAR> csr = CDocLine::GetStringRefWithEOL(pcPageTopLayout->GetDocLineRef());
				CLogicInt	iLogic;
		...
		for( i = 0; i < m_bPreview_EnableLines; ++i ){
			...
			*/
			...
			if( m_pPrintSetting->m_bPrintLineNumber ){
				...
				//文字列長
				const auto nLineCols = int(wcsnlen(szLineNum, std::size(szLineNum)));
...
//	CLayoutInt nTabSpace = m_pParentWnd->GetDocument()->m_cLayoutMgr.GetTabSpace(); //	Sep. 23, 2002 genta LayoutMgrの値を使う
	...
	std::span<const WCHAR> cStringLine( pLine, nDocLineLen );
	CColorStrategy* pStrategy = pStrategyStart;
