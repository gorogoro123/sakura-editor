...
/* 用紙の名前を取得 */
void CPrint::GetPaperName( int nPaperSize, std::wstring& szPaperName )
{
	// 2006.08.14 Moca 用紙情報の統合
	const PAPER_INFO* paperInfo = FindPaperInfo( nPaperSize );
	if( nullptr != paperInfo ){
		szPaperName = paperInfo->m_pszName;
	}else{
		szPaperName = LS(STR_ERR_CPRINT03);
	}
}
...
{
	pPrintSetting.m_szPrintSettingName = settingName;		/* 印刷設定の名前 */
	pPrintSetting.m_szPrintFontFaceHan = L"ＭＳ 明朝";		/* 印刷フォント */
	pPrintSetting.m_szPrintFontFaceZen = L"ＭＳ 明朝";		/* 印刷フォント */
	...
	pPrintSetting.m_szHeaderForm[0] = L"$f";
	...
	pPrintSetting.m_szFooterForm[1] = L"- $p -";
