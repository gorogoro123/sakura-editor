...
public:
	...
	*/
	...
	||  Attributes & Operations
	*/
	static WCHAR* GetFuncInfoByID( HINSTANCE hInstance, int nFuncID, std::span<WCHAR> szFuncName, std::span<WCHAR> szFuncNameJapanese );	/* 機能ID→関数名，機能名日本語 */
	static EFunctionCode GetFuncInfoByName( HINSTANCE hInstance, const WCHAR* pszFuncName, std::span<WCHAR> szFuncNameJapanese );	/* 関数名→機能ID，機能名日本語 */
