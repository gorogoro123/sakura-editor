...
//@@@ 2002.01.14 YAZAKI 印刷プレビューをCPrintPreviewに独立させたことによる変更
		...
		case TTN_NEEDTEXT:
			{
				...
				GetTooltipText(m_szToolTipText, lptip->hdr.idFrom);
	...
	case MYWM_GETLINEDATA:	/* 行(改行単位)データの要求 */
	{
		...
		const wchar_t*	pLine = CDocLine::GetDocLineStrWithEOL(GetDocument()->m_cDocLineMgr.GetLine(nLineNum), &nLineLen);
		...
		pLine = CDocLine::GetDocLineStrWithEOL(GetDocument()->m_cDocLineMgr.GetLine(CLogicInt(wParam)), &nLineLen);
		pLine += nLineOffset;
...
//!ツールチップのテキストを取得
void CEditWnd::GetTooltipText(std::span<WCHAR> szBuf, UINT_PTR idFrom) const
{
