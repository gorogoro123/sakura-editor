...
)
{
	...
	case WM_INITDIALOG:
		...
		/* 初期状態を設定 */
		SendMessageW(hwndDlg, WM_COMMAND, (WPARAM)MAKELONG(IDC_CHECK_KEYHELP,BN_CLICKED), 0 );
...
int CPropTypesKeyHelp::GetData( HWND hwndDlg )
{
	...
	for(i = 0; i < MAX_KEYHELP_FILE; i++){
		if( i < nIndex ){
			...
			m_Types.m_KeyHelpArr[i].m_szAbout = szAbout;
			m_Types.m_KeyHelpArr[i].m_szPath = szPath;
		}else{	/* 未登録部分はクリアする */
