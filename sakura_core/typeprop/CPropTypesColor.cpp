...
bool CPropTypesColor::Import( HWND hwndDlg )
{
	...
	for( int i = 0; i < m_Types.m_nColorInfoArrNum; ++i ){
		...
		wcscpy_s( ColorInfoArr[i].m_szName, std::size(ColorInfoArr[i].m_szName), m_Types.m_ColorInfoArr[i].m_szName );
	}
	...
	for( int i = 0; i < m_Types.m_nColorInfoArrNum; ++i ){
		...
		wcscpy_s( m_Types.m_ColorInfoArr[i].m_szName, std::size(m_Types.m_ColorInfoArr[i].m_szName), ColorInfoArr[i].m_szName );
	}
	/* ダイアログデータの設定 color */
	SetData( hwndDlg );
...
//				MYTRACE( L"\n" );
	...
	case WM_RBUTTONDOWN:
		...
		if( DpiScaleX(18) <= xPos && xPos <= rcItem.right - DpiScaleX(29) ){	// 2009.02.22 ryoji 有効範囲の制限追加
			...
			::SendMessageW( ::GetParent( hwnd ), WM_COMMAND, MAKELONG( IDC_LIST_COLORS, LBN_SELCHANGE ), (LPARAM)hwnd );
...
)
{
	...
	case WM_INITDIALOG:
		...
		{
			LOGFONT	lf = {};
			const auto hFont = (HFONT)::SendMessageW(hwndListColor, WM_GETFONT, 0, 0);
...
void CPropTypesColor::SetData( HWND hwndDlg )
{
	...
	::SendMessageW( hwndDlg, WM_COMMAND, MAKELONG( IDC_LIST_COLORS, LBN_SELCHANGE ), (LPARAM)hwndWork );
